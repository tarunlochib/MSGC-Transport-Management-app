...
const VehiclesListPage = () => {
  ...
  }, [vehicles, searchTerm, sortBy, sortOrder, statusFilter]);
  ...
  const filterAndSortVehicles = () => {
    ...
    filtered.sort((a, b) => {
      ...
      if (sortBy === 'vehicleNumber' || sortBy === 'make' || sortBy === 'model' || sortBy === 'driverName') {
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Are you sure you want to delete this vehicle?')) {
      try {
        ...
        alert('Error deleting vehicle. This vehicle may have associated bookings.');
      }
    }
  };
          ...
          <div className="relative z-10 flex flex-col lg:flex-row lg:items-center lg:justify-between">
            <div className="animate-slideIn">
              ...
              <p className="mt-2 text-sm text-gray-600 font-medium">
                Manage and track all vehicle information and maintenance
              </p>
            ...
            <div className="relative p-4 h-full flex flex-col justify-between">
              <div className="flex items-center justify-between">
                <div className="flex-1">
                  <p className="text-xs font-medium text-gray-500 mb-1">Total Vehicles</p>
                  <p className="text-lg font-bold text-gray-900 mb-1">{stats.total}</p>
                  <div className="flex items-center space-x-1">
                    <div className="w-1.5 h-1.5 bg-blue-500 rounded-full animate-pulse"></div>
                    <span className="text-xs text-gray-500">All vehicles</span>
                  </div>
                </div>
        ...
        <div className="animate-slideDown" style={{ animationDelay: '200ms' }}>
          <div className="group relative overflow-hidden rounded-lg bg-white shadow-sm hover:shadow-md transition-all duration-300 transform hover:scale-105 hover:-translate-y-1 border border-gray-100 h-32">
            ...
            <div className="relative p-4 h-full flex flex-col justify-between">
              <div className="flex items-center justify-between">
                <div className="flex-1">
                  <p className="text-xs font-medium text-gray-500 mb-1">Maintenance</p>
                  <p className="text-lg font-bold text-gray-900 mb-1">{stats.maintenance}</p>
                  <div className="flex items-center space-x-1">
                    <div className="w-1.5 h-1.5 bg-yellow-500 rounded-full animate-pulse"></div>
                    <span className="text-xs text-gray-500">In service</span>
                  </div>
                </div>
              ...
              <div className="w-full bg-gray-200 rounded-full h-1">
                <div className="h-1 bg-yellow-500 rounded-full transition-all duration-1000 ease-out" style={{ width: `${stats.total > 0 ? (stats.maintenance / stats.total) * 100 : 0}%` }}></div>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-2">
              ...
              <div>
                ...
                <p className="text-xs text-gray-500">Find and filter vehicles</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="bg-gradient-to-r from-gray-50 to-gray-100">
              <tr>
                <SortableHeader field="vehicleNumber">Vehicle</SortableHeader>
                <SortableHeader field="make">Make & Model</SortableHeader>
              ...
              {paginatedVehicles.length === 0 ? (
                <tr>
                  <td colSpan="7" className="px-4 py-12 text-center">
                    <div className="flex flex-col items-center animate-fadeIn">
                      ...
                      <h3 className="text-base font-semibold text-gray-900 mb-1">No vehicles found</h3>
                      <p className="text-sm text-gray-500 max-w-sm">Get started by adding your first vehicle.</p>
                    </div>
                  </td>
                </tr>
              ) : (
                paginatedVehicles.map((vehicle, index) => (
                  ...
                  >
                    <td className="px-4 py-3">
                      <div className="flex items-center">
                        ...
                        <div>
                          ...
                          <div className="text-xs text-gray-500">{vehicle.type || 'No type'}</div>
                        </div>
                      </div>
                    </td>
                    <td className="px-4 py-3">
                      <div className="text-sm font-medium text-gray-900">{vehicle.make} {vehicle.model}</div>
                      <div className="text-xs text-gray-500">{vehicle.year || 'No year'}</div>
                    </td>
                    <td className="px-4 py-3">
                    ...
                    <td className="px-4 py-3">
                      <div className="text-sm text-gray-900">{vehicle.driverName || 'No driver'}</div>
                      <div className="text-xs text-gray-500">{vehicle.driverPhone || ''}</div>
                    </td>
                    <td className="px-4 py-3">
                      <div className="text-sm text-gray-900">{vehicle.registrationNumber || 'Not specified'}</div>
                      <div className="text-xs text-gray-500">{formatDate(vehicle.registrationExpiry)}</div>
                    </td>
                    <td className="px-4 py-3">
                      <div className="text-sm text-gray-900">{vehicle.insuranceNumber || 'Not specified'}</div>
                      <div className="text-xs text-gray-500">{formatDate(vehicle.insuranceExpiry)}</div>
                    </td>
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-3">
                <p className="text-xs text-gray-700">
                  ...
                  <span className="font-semibold text-gray-900">{Math.min(startIndex + itemsPerPage, filteredVehicles.length)}</span> of{' '}
                  <span className="font-semibold text-gray-900">{filteredVehicles.length}</span> results
                </p>
              </div>
