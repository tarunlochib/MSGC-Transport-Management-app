...
const EditIncomePage = () => {
  ...
  const fetchData = async () => {
    try {
      const [incomeRes, transportersRes] = await Promise.all([
      ...
      setFormData({
        ...
        transporterId: income.transporterId || ''
      });
      
      setTransporters(transportersRes.data);
    } catch (error) {
      console.error('Error fetching data:', error);
      alert('Error loading income record. Please try again.');
      navigate('/income');
    } finally {
      setLoading(false);
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (error.response?.data?.error) {
        ...
        alert('Error updating income record. Please try again.');
      }
    } finally {
      setSubmitting(false);
    }
            ...
            <div className="relative z-10 flex flex-col lg:flex-row lg:items-center lg:justify-between">
              <div className="animate-slideIn">
                ...
                <p className="mt-2 text-sm text-gray-600 font-medium">
                  Update income record information
                </p>
