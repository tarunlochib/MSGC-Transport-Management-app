...
const BillingStats = ({ stats, loading }) => {
  ...
  // Calculate real percentage changes month-over-month
  ...
  const getChangeType = (currentValue, previousValue = 0) => {
    if (!previousValue || previousValue === 0) return currentValue > 0 ? 'positive' : 'neutral';
  ...
  const billingMetrics = [
    {
      ...
      change: calculatePercentageChange(stats?.totalCommission || 0, stats?.prevTotalCommission || 0),
      changeType: getChangeType(stats?.totalCommission || 0, stats?.prevTotalCommission || 0),
      icon: (
    ...
    },
    {
      ...
      change: calculatePercentageChange(stats?.totalBookings || 0, stats?.prevTotalBookings || 0),
      changeType: getChangeType(stats?.totalBookings || 0, stats?.prevTotalBookings || 0),
      icon: (
        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    },
    {
      ...
      change: calculatePercentageChange(stats?.totalWeight || 0, stats?.prevTotalWeight || 0),
      changeType: getChangeType(stats?.totalWeight || 0, stats?.prevTotalWeight || 0),
      icon: (
        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    },
    {
      ...
      change: calculatePercentageChange(stats?.averageCommission || 0, stats?.prevAverageCommission || 0),
      changeType: getChangeType(stats?.averageCommission || 0, stats?.prevAverageCommission || 0),
      icon: (
        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
